...
final class EventTests: XCTestCase {
    ...
    func test1() {
        ...
        var e: Event? = Event.fromName("公历生日")

        var d: SolarDay? = e?.getSolarDay(2008)
        XCTAssertEqual("2008年2月29日", d?.description)

        // 2005年没有2月29，按最初设置的，没有就倒推1天
        d = e?.getSolarDay(2005)
        XCTAssertEqual("2005年2月28日", d?.description)

        e = Event.fromName("农历生日")
        d = e?.getSolarDay(2026)
        XCTAssertEqual("2026年6月6日", d?.description)
    }
    
    func test2() {
        initData()
        
        let e: Event? = Event.fromName("国际消费者权益日")

        let d: SolarDay? = e?.getSolarDay(2026)
        XCTAssertEqual("2026年3月15日", d?.description)
    }
}
